...
with_other_tags = [f for f in city['features']
                   if 'other_tags' in f['properties'].keys()]
...
with_amenity = [
    f for f in with_other_tags if 'amenity' in f['properties']['other_tags'].keys()]
...
def retreive(tag, value=None):
    res = [f for f in with_other_tags if tag in f['properties']['other_tags'].keys()]
